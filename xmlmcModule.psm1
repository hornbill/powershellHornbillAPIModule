...
# 
# By not providing this parameter, the module will use the credentials of 
#
#.EXAMPLE
# Set-Proxy "http://yourproxyaddress:80/"
# The above will route the requests through your proxy, and will authenticate using the current user details
#
# Set-Proxy "http://yourproxyaddress:80/" "DOMAIN01\User01"
    ...
    try {
        ...
        if($script:ProxyURI -and $script:ProxyURI -ne ""){
            if($script:ProxyCreds) {
                $script:ProxyCreds
                $r = Invoke-WebRequest -Uri $script:URI -Method Post -Headers $script:headers -ContentType "text/xmlmc" -Body $script:body -ErrorAction:Stop -Proxy $script:ProxyURI -ProxyCredential $script:ProxyCreds
            } else {
                $r = Invoke-WebRequest -Uri $script:URI -Method Post -Headers $script:headers -ContentType "text/xmlmc" -Body $script:body -ErrorAction:Stop -Proxy $script:ProxyURI -ProxyUseDefaultCredentials
            }
        } else {
